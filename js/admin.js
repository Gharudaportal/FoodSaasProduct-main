import{c as F}from"./chunks/index-CkZU_ZFb.js";const S="https://hfqmlmfmfnsifwjkjlix.supabase.co",L="eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJpc3MiOiJzdXBhYmFzZSIsInJlZiI6ImhmcW1sbWZtZm5zaWZ3amtqbGl4Iiwicm9sZSI6ImFub24iLCJpYXQiOjE3ODMxODA5MTgsImV4cCI6MjA5ODc1NjkxOH0.0HqMpiYA4cbrJswZ-XIdAF2eH26kTc-KvUIIrE0wKCk",m=F(S,L);let b=localStorage.getItem("adminLoggedIn")==="true",y=[],p=[],g=[],v=[],i={};function N(){const t=window.location.pathname,e=t.includes("login.html")||t.endsWith("/login");return!b&&!e?(window.location.href="login.html",!1):!0}async function M(t){t.preventDefault();const e=document.getElementById("adminUsername").value.trim(),o=document.getElementById("adminPassword").value;try{const{data:n,error:a}=await m.from("admin_users").select("password").eq("username",e).maybeSingle();if(a)throw a;if(!n)throw new Error("User not found");o===n.password?(localStorage.setItem("adminLoggedIn","true"),b=!0,c("Login successful!","success"),setTimeout(()=>{window.location.href="dashboard.html"},500)):c("Invalid username or password","error")}catch(n){console.error(n),c("Login failed – see console","error")}}function x(){localStorage.removeItem("adminLoggedIn"),b=!1,window.location.href="login.html"}async function k(){try{const[t,e,o,n,a]=await Promise.all([m.from("menu_items").select("*").order("id",{ascending:!0}),m.from("categories").select("*").order("id",{ascending:!0}),m.from("orders").select("*").order("created_at",{ascending:!1}),m.from("customers").select("*").order("created_at",{ascending:!1}),m.from("settings").select("*").eq("id",1).maybeSingle()]);t.data&&(y=t.data.map(s=>({id:s.id,name:s.name,description:s.description,price:parseFloat(s.price),category:s.category,isVeg:s.is_veg,isAvailable:s.is_available,image:s.image,rating:parseFloat(s.rating)}))),e.data&&(p=e.data),o.data&&(g=o.data.map(s=>({id:s.id,orderId:s.order_id,customerName:s.customer_name,mobile:s.mobile,address:s.address,landmark:s.landmark,notes:s.notes,paymentMethod:s.payment_method,status:s.status,items:typeof s.items=="string"?JSON.parse(s.items):s.items,total:parseFloat(s.total),date:s.date,time:s.time}))),n.data&&(v=n.data),a.data&&(i=a.data),O(),h(),w(),E(),Z(),X()}catch(t){console.error("Error loading admin data:",t),c("Failed to load data. Please check your connection.","error")}}function O(){const t=new Date().toISOString().split("T")[0],e=g.filter(d=>d.date===t),o=g.filter(d=>d.status!=="Delivered"),n=g.filter(d=>d.status==="Delivered"),a=g.reduce((d,r)=>d+r.total,0);[{id:"todayOrders",value:e.length,icon:"fa-shopping-bag",color:"primary"},{id:"revenue",value:"₹"+a.toLocaleString(),icon:"fa-rupee-sign",color:"success"},{id:"pendingOrders",value:o.length,icon:"fa-clock",color:"warning"},{id:"deliveredOrders",value:n.length,icon:"fa-check-circle",color:"danger"}].forEach(d=>{const r=document.getElementById(d.id);r&&(r.textContent=d.value)});const u=document.getElementById("recentOrders");if(u){const d=g.slice(0,5);u.innerHTML=d.map(r=>`
    ...
    `),o.document.close(),o.print()}function _(){const t=document.getElementById("searchOrders").value.toLowerCase(),e=document.getElementById("ordersTable");if(!e)return;const o=g.filter(n=>n.orderId.toLowerCase().includes(t)||n.customerName.toLowerCase().includes(t)||n.mobile.includes(t));e.innerHTML=o.map(n=>`
        <tr>
            <td>${n.orderId}</td>
    ...
    `}).join(""))}function z(t){const e=v[t],o=g.filter(s=>s.mobile===e.mobile),n=document.getElementById("viewCustomerContent");if(!n)return;n.innerHTML=`
        ...
        `}
    `;const a=document.getElementById("viewCustomerModal");a&&new bootstrap.Modal(a).show()}function X(){if(!i.business_name||!document.getElementById("settingsForm"))return;document.getElementById("settingBusinessName").value=i.business_name,document.getElementById("settingTagline").value=i.tagline||"",document.getElementById("settingContact").value=i.contact_number||"",document.getElementById("settingWhatsApp").value=i.whatsapp_number||"",document.getElementById("settingAddress").value=i.address||"",document.getElementById("settingEmail").value=i.email||"";const e=i.social_media;e&&typeof e=="object"&&(document.getElementById("settingFacebook").value=e.facebook||"",document.getElementById("settingInstagram").value=e.instagram||"",document.getElementById("settingTwitter").value=e.twitter||"")}async function T(t){t.preventDefault();const e=document.getElementById("settingBusinessName").value.trim(),o=document.getElementById("settingTagline").value.trim(),n=document.getElementById("settingContact").value.trim(),a=document.getElementById("settingWhatsApp").value.trim(),s=document.getElementById("settingAddress").value.trim(),u=document.getElementById("settingEmail").value.trim(),d={facebook:document.getElementById("settingFacebook").value.trim(),instagram:document.getElementById("settingInstagram").value.trim(),twitter:document.getElementById("settingTwitter").value.trim()};try{const{data:r,error:f}=await m.from("settings").upsert({id:1,business_name:e,tagline:o,contact_number:n,whatsapp_number:a,address:s,email:u,social_media:d,updated_at:new Date().toISOString()});if(f)throw f;i={...i,business_name:e,tagline:o,contact_number:n,whatsapp_number:a,address:s,email:u,social_media:d,updated_at:new Date().toISOString()},i.business_name=e,i.tagline=o,i.contact_number=n,i.whatsapp_number=a,i.address=s,i.email=u,i.social_media=d,c("Settings saved successfully!","success")}catch(r){console.error("Error saving settings:",r),c("Failed to save settings","error")}}function c(t,e="info"){const o=document.querySelector(".toast-container")||Y(),n=document.createElement("div");n.className=`custom-toast ${e}`;const a={success:"fa-check-circle",error:"fa-times-circle",warning:"fa-exclamation-triangle",info:"fa-info-circle"};n.innerHTML=`<i class="fas ${a[e]||a.info}"></i><span>${t}</span>`,o.appendChild(n),setTimeout(()=>{n.style.animation="slideOut 0.3s ease forwards",setTimeout(()=>n.remove(),300)},3e3)}function Y(){const t=document.createElement("div");return t.className="toast-container",document.body.appendChild(t),t}function K(){var t;(t=document.querySelector(".admin-sidebar"))==null||t.classList.toggle("show")}document.addEventListener("DOMContentLoaded",function(){window.location.pathname.includes("login.html")||N(),k();const t=document.getElementById("loginForm");t&&t.addEventListener("submit",M);const e=document.getElementById("categoryForm");e&&e.addEventListener("submit",C);const o=document.getElementById("menuForm");o&&o.addEventListener("submit",A);const n=document.getElementById("settingsForm");n&&n.addEventListener("submit",T);const a=document.getElementById("orderStatusFilter");a&&a.addEventListener("change",E);const s=document.getElementById("searchOrders");s&&s.addEventListener("input",_)});window.login=M;window.logout=x;window.showAddCategoryModal=j;window.editCategory=q;window.saveCategory=C;window.deleteCategory=P;window.showAddMenuModal=H;window.editMenuItem=R;window.saveMenuItem=A;window.deleteMenuItem=V;window.updateOrderStatus=J;window.viewOrder=U;window.printOrder=W;window.searchOrders=_;window.viewCustomerOrders=z;window.saveSettings=T;window.toggleSidebar=K;
